...
class PremiumCache:
    ...
    def get_stats(self):
        """Vraća statistiku keša sa detaljnim prikazom vremena"""
        age_seconds = time.time() - self.cache.get('timestamp', 0)

        # Izračunaj sve vremenske jedinice
        minutes = age_seconds / 60
        hours = minutes / 60
        days = hours / 24
        weeks = days / 7
        months = days / 30.44  # Prosečan broj dana u mesecu
        years = days / 365.25  # Uzima u obzir prestupne godine
...
class PythonLineCounter:
    ...
    def scan(self, use_cache=True, analyze_complexity=False):
        ...
        print(
            f"{Color.BOLD}{Color.BRIGHT_CYAN}║                    🔍 SKENIRANJE PYTHON FAJLOVA                    ║{Color.RESET}")
        print(
            f"{Color.BOLD}{Color.BRIGHT_CYAN}╚════════════════════════════════════════════════════════════════╝{Color.RESET}")
        ...
        print(
            f"{Color.CYAN}   💾 Ukupna veličina: {Color.WHITE}{self.total_size / 1024:.2f} KB ({self.total_size / (1024 * 1024):.3f} MB){Color.RESET}")
    ...
    def _display_statistics(self):
        ...
        if self.config.get('show_sparklines', True):
            sizes = [f['size'] / 1024 for f in self.files[:100]]
        ...
        if len(name) > 30:
            name = name[:27] + "..."
        print(
            f"│   • Najviše linija:     {Color.YELLOW}{largest_lines['lines']:>8,}{Color.RESET}  {Color.WHITE}{name}{Color.RESET}")
        ...
        print(
            f"│   • Najveći fajl:       {Color.BLUE}{size_kb:>8.2f} KB{Color.RESET}  {Color.WHITE}{name}{Color.RESET}")
        ...
        if len(name) > 30:
            name = name[:27] + "..."
        print(
            f"│   • Najmanje linija:    {Color.GRAY}{smallest_lines['lines']:>8,}{Color.RESET}  {Color.WHITE}{name}{Color.RESET}")
        ...
        print(
            f"│   • Najmanji fajl:      {Color.GRAY}{size_kb:>8.2f} KB{Color.RESET}  {Color.WHITE}{name}{Color.RESET}")

        # Performanse sa detaljnim vremenom keša
        if self.start_time:
            scan_time = time.time() - self.start_time
            ...
            # Detaljan prikaz starosti keša u tabeli
            age = cache_stats['age']

            # Odredi boju na osnovu starosti
            if age['hours'] < 1:
                age_color = Color.GREEN
            elif age['days'] < 1:
                age_color = Color.CYAN
            elif age['days'] < 7:
                age_color = Color.YELLOW
            elif age['days'] < 30:
                age_color = Color.MAGENTA
            else:
                age_color = Color.RED

            print(f"│   {Color.BOLD}⏱️  STAROST KEŠA:{Color.RESET}")
            print(f"│   ┌─────────────────────────────────────────────────────────┐")
            print(f"│   │ {Color.BOLD}Jedinica     Vrednost{' ' * 15}{Color.RESET} │")
            print(f"│   ├─────────────────────────────────────────────────────────┤")
            print(f"│   │ Sekunde      {age_color}{age['seconds']:>23.2f}{Color.RESET} s   │")
            print(f"│   │ Minuti       {age_color}{age['minutes']:>23.2f}{Color.RESET} min │")
            print(f"│   │ Sati         {age_color}{age['hours']:>23.2f}{Color.RESET} h   │")
            print(f"│   │ Dani         {age_color}{age['days']:>23.2f}{Color.RESET} d   │")
            print(f"│   │ Nedelje      {age_color}{age['weeks']:>23.2f}{Color.RESET} ned │")
            print(f"│   │ Meseci       {age_color}{age['months']:>23.2f}{Color.RESET} mes │")
            print(f"│   │ Godine       {age_color}{age['years']:>23.2f}{Color.RESET} god │")
            print(f"│   └─────────────────────────────────────────────────────────┘")

            # Dodaj vizuelni indikator starosti
            if age['hours'] < 1:
                status = f"{Color.GREEN}● SVEŽ{Color.RESET}"
            elif age['days'] < 1:
                status = f"{Color.CYAN}● AKTIVAN{Color.RESET}"
            elif age['days'] < 7:
                status = f"{Color.YELLOW}● UMEREN{Color.RESET}"
            elif age['days'] < 30:
                status = f"{Color.MAGENTA}● ZASTAREO{Color.RESET}"
            else:
                status = f"{Color.RED}● VRLO ZASTAREO{Color.RESET}"

            # Progress bar za vizuelizaciju starosti (pretpostavljajući da je max 30 dana)
            max_days = 30
            days_progress = min(age['days'], max_days)
            progress_bar = PremiumVisualizer.progress_bar(
                days_progress,
                max_days,
                width=20,
                label=f"│   • Status keša:",
    ...
    def display_duplicates(self):
        ...
        for i, dup in enumerate(duplicates[:10], 1):
            print(
                f"│ {Color.BOLD}Grupa {i}:{Color.RESET} {Color.CYAN}{dup['hash']}{Color.RESET} ({dup['count']} kopije, {dup['size']:.1f} KB)")
        ...
        if total_duplicate_size > 0:
            print(
                f"{Color.CYAN}   💾 Potencijalna ušteda: {total_duplicate_size:.1f} KB ({total_duplicate_size / 1024:.2f} MB){Color.RESET}")
...
{Color.BRIGHT_MAGENTA}💾 KEŠ SISTEM:{Color.RESET}
  ...
  python counter.py --cache-stats     # Prikaži statistiku keša
    ...
    if args.config:
        ...
        for key, value in config.config.items():
            if isinstance(value, dict):
                print(
                    f"│ {Color.YELLOW}{key:25}{Color.RESET} = {Color.WHITE}{json.dumps(value, ensure_ascii=False)[:50]}{Color.RESET}")
            else:
    ...
    if args.cache_stats:
        ...
        print("├─────────────────────────────────────────────────────────────────┤")
        print(f"│ {Color.BOLD}⏱️  STAROST KEŠA - SVE JEDINICE{Color.RESET}")
        print("│   ┌─────────────────────────────────────────────────────────────┐")
        print("│   │ Jedinica     |    Vrednost |      Prikaz                    │")
        print("│   ├─────────────────────────────────────────────────────────────┤")
        print(
            f"│   │ Sekunde      | {Color.CYAN}{age['seconds']:>11.2f}{Color.RESET} s | {age['seconds']:.1f} sekundi      │")
        print(
            f"│   │ Minuti       | {Color.CYAN}{age['minutes']:>11.2f}{Color.RESET} min | {age['minutes']:.1f} minuta        │")
        print(
            f"│   │ Sati         | {Color.CYAN}{age['hours']:>11.2f}{Color.RESET} h   | {age['hours']:.1f} sati          │")
        print(
            f"│   │ Dani         | {Color.CYAN}{age['days']:>11.2f}{Color.RESET} d   | {age['days']:.1f} dana          │")
        print(
            f"│   │ Nedelje      | {Color.CYAN}{age['weeks']:>11.2f}{Color.RESET} ned | {age['weeks']:.1f} nedelja       │")
        print(
            f"│   │ Meseci       | {Color.CYAN}{age['months']:>11.2f}{Color.RESET} mes | {age['months']:.1f} meseci       │")
        print(
            f"│   │ Godine       | {Color.CYAN}{age['years']:>11.2f}{Color.RESET} god | {age['years']:.1f} godina       │")
        print("│   └─────────────────────────────────────────────────────────────┘")

        # Prirodni jezik opis starosti
        if age['seconds'] < 60:
            desc = f"Keš je star {Color.GREEN}{age['seconds']:.1f} sekundi{Color.RESET}"
        elif age['minutes'] < 60:
            desc = f"Keš je star {Color.CYAN}{age['minutes']:.1f} minuta{Color.RESET}"
        elif age['hours'] < 24:
            desc = f"Keš je star {Color.YELLOW}{age['hours']:.1f} sati{Color.RESET}"
        elif age['days'] < 7:
            desc = f"Keš je star {Color.MAGENTA}{age['days']:.1f} dana{Color.RESET}"
        elif age['days'] < 30:
            desc = f"Keš je star {Color.RED}{age['weeks']:.1f} nedelja{Color.RESET}"
        elif age['days'] < 365:
            desc = f"Keš je star {Color.RED}{age['months']:.1f} meseci{Color.RESET}"
        else:
            desc = f"Keš je star {Color.RED}{age['years']:.1f} godina{Color.RESET}"
    ...
    try:
        ...
        if cache_stats['files'] > 0:
            print(
                f"{Color.DIM}   💾 Keš: {cache_stats['files']} fajlova, {cache_stats['age']['days']:.1f} dana starosti{Color.RESET}")
