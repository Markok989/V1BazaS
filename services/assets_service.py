...
# (FILENAME: services/assets_service.py - START PART 1/4)
# -*- coding: utf-8 -*-
"""
...
Napomena:
- Fail-closed: bez permisije nema podataka.
- SQLite fallback koristi isti DB path kao core.db (sprečava scenario "dve baze").

Rev (senior patch):
- RBAC bugfix: list_assets sada radi i za METRO/MY (uz scope filtriranje), ne samo za FULL.
...
# (FILENAME: services/assets_service.py - END PART 1/4)

# FILENAME: services/assets_service.py
# (FILENAME: services/assets_service.py - START PART 2/4)
# -------------------- MY scope helpers --------------------
...
def _call_compatible(func, **kwargs):
    ...
    - prihvati kwargs ako funkcija prima **kwargs
...
# (FILENAME: services/assets_service.py - END PART 2/4)

# FILENAME: services/assets_service.py
# (FILENAME: services/assets_service.py - START PART 3/4)
...
# (FILENAME: services/assets_service.py - END PART 3/4)

# FILENAME: services/assets_service.py
# (FILENAME: services/assets_service.py - START PART 4/4)
# -------------------- Metrology-scope --------------------
...
def _metrology_scope_filter(rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    # Ako postoji DB helper, čuvamo postojeću logiku (per-row).
    if _db_asset_has_metrology is not None:
...
def _apply_list_scope(rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    Centralizovano scope filtriranje za list_* pozive.
    Precedenca: FULL -> METRO -> MY.
    ...
    if _has_full_assets_view():
        out = rows
    elif _has_metro_assets_view():
        out = _metrology_scope_filter(rows)
    elif _has_my_assets_view():
...
def list_assets_my(
    *,
    ...
    if _db_list_assets is not None:
        payload = dict(q=final_q, search=final_q, category=(category or "SVE"), status=(status or "SVE"), limit=lim, sector="")
        try:
            res = _call_compatible(_db_list_assets, **payload)
        except Exception as e:
            logger.exception("core.db.list_assets (MY) failed: %s", e)
            res = []
        if not isinstance(res, list):
            try:
                res = list(res)  # type: ignore
    ...
    try:
        rows2 = _sqlite_list_assets(final_q=final_q, category=category, status=status, limit=lim)
        return _my_scope_filter(rows2)
    except Exception as e:
        logger.exception("sqlite list_assets_my failed: %s", e)
        return []


# alias (kompatibilnost)
list_assets_my_brief = list_assets_my
# (FILENAME: services/assets_service.py - END PART 4/4)